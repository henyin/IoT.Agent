package com.ydttech.iot.vo.config;
...
public class IBGSConfig {
    ...
    private static Logger logger = LoggerFactory.getLogger(IBGSConfig.class);

    public static String initCfgDirectory = System.getProperty("user.dir") + "//..//conf//";
    public static String initCfgFilename = "envSIT.xml";
//    public static String initCfgFilename = "envSIT.xml";
    ...
    public static String LPRCFG_IMAGEFOLDER;
    ...
    private static void loadIBGSConfig() throws DocumentException {
        ...
        List<Node> nodeList;
            ...
            if (System.getProperty("initCfg") != null) {
                initCfg = System.getProperty("initCfg");
            } else {
                initCfg = initCfgDirectory.concat(initCfgFilename);
            }

            logger.info("ibgs config file name:{}", initCfg);
            docCfgXml = reader.read(new File(initCfg));

//        docCfgXml = reader.read(IBGSConfig.class.getResourceAsStream(initCfgFilename));


        node = docCfgXml.selectSingleNode("//lprCfg");
            if (node.hasContent()) {
                LPRCFG_IMAGEFOLDER = node.valueOf("imageFolder");
            } else {
                logger.error("loadIBGSConfig LPRCFG no content!");
                System.exit(1);
            }
